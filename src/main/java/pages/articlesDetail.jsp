<%-- 
    Document   : articlesDetail
    Created on : Jan 13, 2015, 7:01:33 PM
    Author     : shabil.b
--%>
...
try {
    ...
    String news = request.getParameter("a") == null ? "" : request.getParameter("a").replaceAll("\\<.*?>", "");
                                ...
                                if (articlesList != null) {
                                    for (Articles ar : articlesList) {
                                        if (ar.getLanguageId().equals(language.getId())) {
                                            if (ar.getParentId().equals(news)) {%>
                                                <li>
                                                    <p><%=ar.getTitle()%></p>
                                                    <p class=""><%=ar.getDescription()%></p>
                                                </li>
                                            <%}
                                        }
                                    }
...
<%} catch (Throwable e) {
    Logger.getLogger("articlesDetail.jsp").log(Level.SEVERE, null, e);
}%>