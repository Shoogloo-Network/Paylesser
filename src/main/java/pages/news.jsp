<%-- 
    Document   : news
    Created on : Jan 12, 2015, 2:24:39 PM
    Author     : shabil.b
--%>
...
<%}%>				
            <title itemprop="name"><%=p.getProperty("public.news.title")%></title>
        ...
        <body>
            ...
            <div class="page-content clearfix inner-page static-pages">
                <div class="main-offers-wrap wrapper clearfix">
                    <div class="left-col">
                        <h2 class="styled-bg"><%=p.getProperty("public.news.header")%></h2>
                        <div class="about-us custom-content">
                            <ul>
                                <%List<News> newsList = home.getNews(store);
                                if(newsList != null) {
                                    for(News nw : newsList) {
                                        if(nw.getLanguageId().equals(language.getId())) {
                                            newsDesc = nw.getDescription().replaceAll("\\<.*?>", "");
                                          
                                            if(newsDesc.length() > 314) {
                                              dot = "&nbsp;...";
                                            }%>
                                            <li>
                                                <p class="news" onclick="news(<%=store%>, <%=nw.getParentId()%>);"><%=nw.getTitle()%></p>
                                                <p>
                                                  <%if(newsDesc.length() > 314) {
                                                    out.print(newsDesc.substring(0, 315)+dot);
                                                  }
                                                  else {
                                                    out.print(newsDesc);
                                                  }%>
...
<%} catch (Throwable e) {
    Logger.getLogger("news.jsp").log(Level.SEVERE, null, e);
}%>